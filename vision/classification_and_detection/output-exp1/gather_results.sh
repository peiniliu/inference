...
WORKDIR="/gpfs/bsc_home/xpliu/inference/vision/classification_and_detection/output-exp1"

#1.benchmark, 2.env
function create_head () {
        output="${1}-${2}.csv"
        echo "benchmark,k8senv,sbatch,numctn,cscenario,cbatch,reps,qps"  > $output
            ...
            case $scenario in
                 "O")
                    ...
                    do
                         ...
                         echo "$bench,$env,$sbatch,$ctn,offline,$cbatch,$rep,$qps" >> $output
                    done
                 ;;
                 "S")
