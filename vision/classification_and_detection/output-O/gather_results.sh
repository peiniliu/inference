            ...
            case $scenario in
                 ...
                 "S")
                    ...
                    do
                         ...
                         p99=`grep \"99.0\" $FILE | awk -F':' '{print $2}' | awk -F',' '{print $1}'`
                         p999=`grep \"99.9\" $FILE | awk -F':' '{print $2}' | awk -F',' '{print $1}'`
                         echo "$bench,$env,$sbatch,$ctn,server,$cbatch,$rep,$qps,$mean,$p50,$p80,$p90,$p95,$p99,$p999" >> $output
                    done
                 ;;
                 "SS")
                    for cbatch in $LIST_CLIENT_BATCH
                    do
                         ...
                         p99=`grep \"99.0\" $FILE | awk -F':' '{print $2}' | awk -F',' '{print $1}'`
                         p999=`grep \"99.9\" $FILE | awk -F':' '{print $2}' | awk -F',' '{print $1}'`
                         echo "$bench,$env,$sbatch,$ctn,singlestream,$cbatch,$rep,$qps,$mean,$p50,$p80,$p90,$p95,$p99,$p999" >> $output
                    done
                 ;;
                 "MS")
                    for cbatch in $LIST_CLIENT_BATCH
                    do
                         ...
                         p99=`grep \"99.0\" $FILE | awk -F':' '{print $2}' | awk -F',' '{print $1}'`
                         p999=`grep \"99.9\" $FILE | awk -F':' '{print $2}' | awk -F',' '{print $1}'`
                         echo "$bench,$env,$sbatch,$ctn,multistream,$cbatch,$rep,$qps,$mean,$p50,$p80,$p90,$p95,$p99,$p999" >> $output
                    done
                 ;;
            esac
        done
