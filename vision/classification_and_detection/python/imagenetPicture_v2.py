...
log = logging.getLogger("imagenetPicture_v2")
...
class Imagenet(dataset.Dataset):
    ...
    def __init__(self, data_path, image_list, name, use_cache=0, image_size=None,
        ...
        with open(image_list, 'r') as f:
            for s in f:
                ...
                if not os.path.exists(dst):
                    # a preprocessed version of the image(now copy)
        ...
        #log.info("PEINI: init imagenetPicture dataset. image_list={}, label_list={}".format(self.image_list,self.label_list))

    def get_item(self, nr):
        """Get image by number in the list."""
        dst = os.path.join(self.cache_dir, self.image_list[nr])
        log.info(dst)
        #log.info(self.label_list[nr])
        with open(dst, 'rb') as f:
           image = f.read()
        jpeg_rgb = Image.open(io.BytesIO(image))
        img = (np.array(jpeg_rgb) / 255.0).tolist()
        # print(img)
        # img = '{"b64": "%s"}' % jpeg_bytes
        # log.info("PEINI: image read {}".format(img))
