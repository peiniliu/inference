...
SUPPORTED_DATASETS = {
    "imagenet_tfserving_v3":
        (imagenet_pickle.Imagenet, dataset.pre_process_vgg, dataset.PostProcessSeldon(offset=-1),
...
SUPPORTED_PROFILES = {
    ...
    "resnet50-tfserving_v3": {
        "dataset": "imagenet_tfserving_v3",
        "backend": "tfserving_v3",
...
def get_backend(backend):
    ...
    elif backend == "tfserving_v3":
        from backend_tfserving_v3 import BackendTfservingV3
        backend = BackendTfservingV3()
    else:
...
class RunnerBase:
    ...
    def run_one_item(self, qitem):
        ...
        try:
            ...
            processed_results = self.post_process(results.json()['predictions'], qitem.content_id, qitem.label, self.result_dict)
    ...
    def enqueue(self, query_samples):
        ...
        #log.info("enqueue"+str(len(query_samples))+str(idx)+str(query_id))
        if len(query_samples) < self.max_batchsize:
            data, label = self.ds.get_samples(idx)
            # print(json.loads(data))
            self.run_one_item(Item(query_id, idx, data, label))
        else:
            print("over max batch size")
...
class QueueRunner(RunnerBase):
    def __init__(self, model, ds, threads, post_proc=None, max_batchsize=128):
        ...
        for _ in range(self.threads):
            ...
            # worker = multiprocessing.Process(target=self.handle_tasks, args=(self.tasks,))
            worker.daemon = True
    ...
    def enqueue(self, query_samples):
        idx = [q.index for q in query_samples]
        query_id = [q.id for q in query_samples]
        #log.info("enqueuerunner"+str(len(query_samples))+str(idx)+str(query_id))
        if len(query_samples) < self.max_batchsize:
            data, label = self.ds.get_samples(idx)
